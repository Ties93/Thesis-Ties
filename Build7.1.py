...
#force sluice behaviour between 0 and a flexible max
    ...
    def obj(model):
        ...
        def slacks():
            return (sum([model.s_ijm[t]/max(model.s_ijm) + model.s_or[t]/max(model.s_or) + model.s_hout[t]/max(model.s_hout) + model.s_korn[t]/max(model.s_korn) + model.s_den[t]/max(model.s_den) for t in model.T]))/5
...
#opt.options["nlp_scaling_method"] = 'none'#"equilibration-based"#"user-scaling"
...
plt.figure()
plt.plot(MB_nzk1, label='S[i]')
plt.plot(MB_nzk2, label='S[i-1] + dt*(Qin[i-1] - Qout[i-1])')
plt.title('Mass Balance NZK')
plt.figure()
plt.plot(MB_mar1, label='S[i]')
plt.plot(MB_mar2, label='S[i-1] + dt*(Qin[i-1] - Qout[i-1])')
plt.title('Mass Balance MAR')
plt.figure()
plt.plot(MB_ijs1, label='S[i]')
plt.plot(MB_ijs2, label='S[i-1] + dt*(Qin[i-1] - Qout[i-1])')
